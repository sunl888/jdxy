...
$(document).ready(function() { 	
	...
	(function(){
	  // if firefox 3.5+, hide content till load (or 3 seconds) to prevent
		// FOUT
    ...
    $('.search-btn').addClass('nostyle');// tell uniform to not style this
											// element
	...
	// hover magic add blue color to icons when hover - remove or change the
	// class if not you like.
	...
	$( '.collapseBtn' ).bind( 'click', function(){
		...
		if ($this.hasClass('leftbar')) {
			...
			if($(this).hasClass('hide')) {
				...
				$('.collapseBtn.leftbar').animate({ // use .hide() if you
													// experience heavy
													// animation :)
				    left: '200',
				    top: '20'
		...
		if ($this.hasClass('rightbar')) {
			...
			if($(this).hasClass('hide')) {
				...
				/*
				 * if($('#sidebar').length) { $('#sidebar').css({'left' : '0',
				 * 'margin-left' : '0'}); }
				 */
				$this.removeClass('hide');
				...
				$('.collapseBtn.rightbar').animate({ // use .hide() if you
														// experience heavy
														// animation :)
				    right: '10',
				    top: '78'
	...
	// -------- 管理员组值发生改变时，使用Ajax更新权限组值 -------- //
	$('#e8_admin_group').change(function(){
		// 获取当前选中分组ID
		var admin_group_id = $(this).children('option:selected').val();
		$.ajax({
			  type:"get", // 当前提交请求的方式为get
			  url:APP + "/Admin/Admin/getPermGroupList" + "/admin_group_id/" + admin_group_id, // 提交给当前控制器的ajax方法,并附带参数
			  ...
			  success:　function(data){
				  if (data.status == 1) {
					  ...
					  $(data.data).each(function(position){
						  $("#e8_perm_group").append("<option selected='selected' value='"+data.data[position].perm_group_id+"'>"+data.data[position].name+"</option>");
					  });
				  } else {
					  // 清空select
					  $("#e8_perm_group").empty();
					  $("#e8_ajax_msg").html(data.msg);
				  }
			  },
			  // 请求失败调用方法
			  error:function(data){
				  alert("Ajax调用请求失败,请联系管理员");
			  }
			});
	});
	
	// ------------ 当账户文本框失去焦点是调用Ajax验证该账户是否存在 ---------------//
	$('#e8_account').blur(function(){
		var account = $(this).val();
		$.ajax({
			  type:"get", // 当前提交请求的方式为get
			  url:APP + "/Admin/Admin/checkAccount" + "/account/" + account, // 提交给当前控制器的ajax方法,并附带参数
			  dataType: "json", // 返回数据格式为json
			  // 请求成功调用方法
			  success:　function(data){
				  if (data.status == 1) {
	...
	// ------------ 当邮箱文本框失去焦点是调用Ajax验证该账户是否存在 ---------------//
	$('#e8_email').blur(function(){
		var email = $(this).val();
		$.ajax({
			  type:"get", // 当前提交请求的方式为get
			  url:APP + "/Admin/Admin/checkEmail" + "/email/" + email, // 提交给当前控制器的ajax方法,并附带参数
			  dataType: "json", // 返回数据格式为json
			  // 请求成功调用方法
			  success:　function(data){
				  if (data.status == 1) {
	...
	$('#s-timepicker').timeEntry({
		show24Hours: true,
		spinnerImage: ''
	});
	$('#s-timepicker').timeEntry('setTime', '8:00')
	
	$('#e-timepicker').timeEntry({
		show24Hours: true,
		spinnerImage: ''
	});
	$('#e-timepicker').timeEntry('setTime', '22:00')


});

// ------------- Ajax更改AdminGroup的可用状态 -------------//
function changeAdminGroupUseful(admin_group_id,mw){
	$.ajax({
		  type:"get", // 当前提交请求的方式为get
		  url:APP + "/Admin/AdminGroup/changeUseful" + "/id/" + admin_group_id + '/mw/' + mw,
		  dataType: "json", // 返回数据格式为json
		  // 请求成功调用方法
		  success:　function(data){
			  if (data.status == 1) {
...
/**
 * 确认删除对话框 URL:点击确认后跳转的地址 message:提示信息
 */
...
/**
 * 返回确认对话框 URL:点击确认后跳转的地址 message:提示信息
 */
function cancelConfirm(URL,message){
   $.confirm({
		'title'		: '确定返回？',
		'message'	: message,
		'buttons'	: {
...
/**
 * 确认对话框 URL:点击确认后跳转的地址 message:提示信息
 */
function confirm2(URL,message){
   $.confirm({
		'title'		: '确定执行改操作？',
		'message'	: message,
		'buttons'	: {
