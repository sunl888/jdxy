...
class ContentController extends BaseController
{
    ...
    public function add()
    {
        ...
        if (IS_POST) {
            if (!$Content->validate($Content->_validate)->create()) {
                ...
                if ( ($content_id = $Content->add()) ) {
                    //todo This extension was deprecated in PHP 5.5.0, and it was removed in PHP 7.0.0. Instead.
    ...
    public function restore()
    {
        ...
        if ($j > 0) {
            ...
            //todo 这里有个BUG，将待审核文章移动到回收站再恢复直接就变成publish状态了
            //2017/5/17  孙龙  publish->check
            $contentModel->where($where)->setField('state', 'check');
